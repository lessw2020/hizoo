...
class OurTrainer(Trainer):
    ...
    def _inner_training_loop(
        ...
        elif not delay_optimizer_creation: #T
        ...
        if args.gradient_checkpointing: #F
            self.model.gradient_checkpointing_enable()

        model = self._wrap_model(self.model_wrapped)

        if is_sagemaker_mp_enabled() and resume_from_checkpoint is not None: #F
            self._load_from_checkpoint(resume_from_checkpoint, model)

        # for the rest of this function `model` is the outside model, whether it was wrapped or not
        if model is not self.model: #F
            self.model_wrapped = model

        if delay_optimizer_creation: #T
            self.create_optimizer_and_scheduler(num_training_steps=max_steps)

        # Check if saved optimizer or scheduler states exist #T
        ...
        if not args.ignore_data_skip: #F
